...
describe("Test forgot page", () => {
  ...
  it("should send rest email, if email is correct", (browser) => {
    forgotPage.navigate()
      .submit("test@fibo-captcha-app.com")
      .waitForElementPresent(".flash")
      .assert.elementPresent("@flashInfo")
      .assert.containsText("@flashInfo", "An email has been sent to test@fibo-captcha-app.com with further instructions.")
      .api.pause(pauseTime)
      .makeScreenshot();
    ...
    browser
      ...
      .perform(function(browser, done) {
        ...
        let re = /reset\/(\w+)/g;      
        mailtrap.getTokenFromMessage("test@fibo-captcha-app.com", re, function(err, token, message) {
          if (err) 
            throw new Error(err);
