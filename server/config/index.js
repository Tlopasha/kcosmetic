...
if (WEBPACK_BUNDLE) {
  ...
  if (process.argv.length > 0) {
    if (process.argv[1] && process.argv[1].indexOf('pm2') !== -1) {
      bundleFullPath = path.resolve(__dirname);
      isPm2 = true;
    } else {
  ...
  console.log(`bundleFullPath: ${bundleFullPath}, isPm2: ${isPm2}, global.rootPath: `, global.rootPath);
  if (!isPm2) {
    global.rootPath = path.normalize(path.resolve(path.dirname(bundleFullPath), '..'));
  } else {
    global.rootPath = path.normalize(path.resolve(bundleFullPath, '..', '..'));
  }
}
