...
class Services extends EventEmitter {
  ...
  /**
   * Load built-in and biz services. Scan the folders
  ...
  loadServices(app, db) {
    ...
    if (
      WEBPACK_BUNDLE ||
      fs.existsSync(path.join(__dirname, '..', 'biz', 'modules'))
    ) {
      logger.info('');
      logger.info(chalk.bold('Search biz services...'));

      let modules = require.context('../biz/modules', true, /service\.js$/);
      if (modules) {
        modules.keys().map(function(module) {
          logger.info(
            '  Load',
            path.relative(path.join(__dirname, '..', 'biz', 'modules'), module),
            'service...',
          );
          addService(modules(module));
        });
      ...
      if (service.$settings.rest !== false && service.actions) {
        ...
        _.forIn(service.actions, (actionFunc, name) => {
          ...
          if (!_.isFunction(action.handler))
            throw new Error(
              `Missing handler function in '${name}' action in '${
                service.name
              }' service!`,
            );
          ...
          let handler = (req, res) => {
            ...
            logger.debug(
              `Request via REST '${service.namespace}/${action.name}' (ID: ${
                ctx.id
              })`,
              ctx.params,
            );
          ...
          if (action.middlewares && _.isArray(action.middlewares)) {
          ...
          router.get('/' + name, middlewares, handler);
          router.post('/' + name, middlewares, handler);

          // You can call with ID in the path
          //     GET  /api/namespace/123/vote
          //     POST /api/namespace/123/vote
          router.get('/:' + idParamName + '/' + name, middlewares, handler);
          router.post('/:' + idParamName + '/' + name, middlewares, handler);
        ...
        app.use(
          '/exported',
          express.static(path.join(global.rootPath, 'export'), {
            maxAge: 31557600000,
          }),
        );
      }
